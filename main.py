...
<div style="background: rgba(255, 165, 0, 0.15); border-left: 4px solid #ffaa00; padding: 15px; margin-bottom: 20px; border-radius: 4px; font-size: 0.9rem; color: #ffcc80;">
    ...
    This report was generated by an Autonomous AI Agent. Verify all data against primary sources.
</div>
"""
...
# --- MASSIVE REGION MAP (60+ Countries) ---
REGION_MAP = {
    ...
    "Czech Republic": "cz-cs", "Denmark": "dk-da", "Egypt": "xa-ar",
    "Estonia": "ee-et", "Finland": "fi-fi", "France": "fr-fr", "Germany": "de-de",
...
async def get_trending(request: TrendingRequest):
    """
    Fetches real-time trending news for the selected region.
    """
    region_code = REGION_MAP.get(request.region, "wt-wt")
    headlines = []
    try:
        with DDGS() as ddgs:
            # Fetch generic "news" or "trending" for that region
            results = list(ddgs.news("top stories", region=region_code, max_results=4))
            for r in results:
                headlines.append({
                    "title": r['title'],
                    "url": r['url'],
